...
public class GetterRecommendationWeather {
    ...
    public GetterRecommendationWeather(UserRepository userRepository, Long chatId, StoreUnits storeUnits) {
    ...
    public String getRecommendation() throws IOException {
        ...
        ParserRecommendationWeather.RecommendationInfoDto recommendationInfo = parser.parseRecommendationInfo(userRepository, chatId);
        double temperature;
        if(storeUnits.getTemperature().equals(TEMPERATURE.K)) {
            temperature = recommendationInfo.getTemperature() + 273.15;
        } else {
            temperature = recommendationInfo.getTemperature();
        }
